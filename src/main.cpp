...
// Copyright (c) 2017 The Shekel developers
...
#if defined(NDEBUG)
#error "Shekel cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Shekel
...
{
    RenameThread("shekel-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZJEWMinted()
{
...
void RecalculateZJEWSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zJEW supply
...
bool RecalculateJEWSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
        RecalculateZJEWMinted();
        RecalculateZJEWSpent();
        RecalculateJEWSupply(1);
    }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert JEW to zJEW
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Shekel: We use certain sporks during IBD, so check to see if they are
